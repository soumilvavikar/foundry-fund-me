...
import {UnderstandingStorage} from "../src/UnderstandingStorage.sol";

contract DeployUnderstandingStorage is Script {
    function run() external returns (UnderstandingStorage) {
        vm.startBroadcast();
        UnderstandingStorage understandingStorage = new UnderstandingStorage();
        vm.stopBroadcast();
        printStorageData(address(understandingStorage));
        printFirstArrayElement(address(understandingStorage));
        return (understandingStorage);
    }
